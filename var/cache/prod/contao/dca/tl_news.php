...
class tl_news extends \Contao\Backend
{
    ...
    public function getSourceOptions(\Contao\DataContainer $dc)
    {
        ...
        if ($security->isGranted(\Contao\CoreBundle\Security\ContaoCorePermissions::USER_CAN_EDIT_FIELD_OF_TABLE, 'tl_news::jumpTo')) {
            $arrOptions[] = 'internal';
        }
        // Add the "article" option
        if ($security->isGranted(\Contao\CoreBundle\Security\ContaoCorePermissions::USER_CAN_EDIT_FIELD_OF_TABLE, 'tl_news::articleId')) {
            $arrOptions[] = 'article';
        }
        // Add the "external" option
        if ($security->isGranted(\Contao\CoreBundle\Security\ContaoCorePermissions::USER_CAN_EDIT_FIELD_OF_TABLE, 'tl_news::url')) {
            $arrOptions[] = 'external';
        }
    ...
    {
        $archiveModel = \Contao\NewsArchiveModel::findByPk($dc->activeRecord->pid);
        if ($archiveModel === \null) {
            return $tags;
        }
