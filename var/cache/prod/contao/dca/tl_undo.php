...
$GLOBALS['TL_DCA']['tl_undo'] = array(
    // Config
    'config' => array('dataContainer' => \Contao\DC_Table::class, 'closed' => \true, 'notEditable' => \true, 'sql' => array('keys' => array('id' => 'primary')), 'onload_callback' => array(array('tl_undo', 'checkPermission')), 'onshow_callback' => array(array('tl_undo', 'showDeletedRecords'))),
    // List
    'list' => array('sorting' => array('mode' => \Contao\DataContainer::MODE_SORTABLE, 'fields' => array('tstamp DESC'), 'panelLayout' => 'filter;sort,search,limit'), 'label' => array('fields' => array('tstamp', 'pid', 'fromTable', 'query')), 'operations' => array('undo' => array('href' => '&amp;act=undo', 'icon' => 'undo.svg'), 'jumpToParent' => array('icon' => 'parent.svg'), 'show' => array('href' => '&amp;act=show', 'icon' => 'show.svg'))),
    // Fields
...
class tl_undo extends \Contao\Backend
{
    ...
    public function checkPermission()
    {
        ...
        $objSteps = $this->Database->prepare("SELECT id FROM tl_undo WHERE pid=?")->execute($this->User->id);
        // Restrict the list
        $GLOBALS['TL_DCA']['tl_undo']['list']['sorting']['root'] = $objSteps->numRows ? $objSteps->fetchEach('id') : array(0);
        // Redirect if there is an error
        if (\Contao\Input::get('act') && !\in_array(\Contao\Input::get('id'), $GLOBALS['TL_DCA']['tl_undo']['list']['sorting']['root'] ?? array())) {
