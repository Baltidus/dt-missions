...
class tl_content extends \Contao\Backend
{
    ...
    public function preserveReferenced()
    {
        if (\Contao\Input::get('act') == 'delete') {
            $objCes = $this->Database->prepare("SELECT COUNT(*) AS cnt FROM tl_content WHERE type='alias' AND cteAlias=? AND (ptable='tl_article' OR ptable='')")->execute(\Contao\Input::get('id'));
            if ($objCes->cnt > 0) {
        ...
        if (\Contao\Input::get('act') == 'deleteAll') {
            $objCes = $this->Database->prepare("SELECT cteAlias FROM tl_content WHERE type='alias' AND (ptable='tl_article' OR ptable='')")->execute();
            $objSession = \Contao\System::getContainer()->get('session');
            $session = $objSession->all();
...
' . \Contao\StringUtil::insertTagToSrc($this->getContentElement($objModel)) . '
    ...
    public function getForms()
    {
        ...
        $security = \Contao\System::getContainer()->get('security.helper');
        while ($objForms->next()) {
            if ($security->isGranted(\Contao\CoreBundle\Security\ContaoCorePermissions::USER_CAN_ACCESS_FORM, $objForms->id)) {
    ...
    {
        // Disable the button if the element type is not allowed
        if (!\Contao\System::getContainer()->get('security.helper')->isGranted(\Contao\CoreBundle\Security\ContaoCorePermissions::USER_CAN_ACCESS_ELEMENT_TYPE, $row['type'])) {
            return \Contao\Image::getHtml(\preg_replace('/\\.svg$/i', '_.svg', $icon)) . ' ';
        }
        $objElement = $this->Database->prepare("SELECT id FROM tl_content WHERE type='alias' AND cteAlias=?")->limit(1)->execute($row['id']);
